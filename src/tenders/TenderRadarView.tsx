...
export function TenderRadarView({ data, refresh, request, navigate, moduleNavigation }: TenderRadarViewProps) {
  ...
  const load = async () => {
    ...
    try {
      const [radar, loadedProfiles] = await Promise.all([loadRadar<TenderRadarPayload>(request), loadProfiles<TenderSearchProfile[]>(request)]);
      setPayload(radar); setProfiles(loadedProfiles);
  ...
  return <section className="stack tenders-page tender-radar-view" aria-labelledby="tender-radar-heading">
    ...
    <TenderSavedSearches filters={filters} profiles={profiles} request={request} onProfilesChange={setProfiles} onApply={applyProfile} />
