...
export function TenderDocumentSection({ documents, busy, statusText, sourceUrl, refreshResult, onRefresh, onUpload, documentTypeLabel = defaultTypeLabel }: TenderDocumentSectionProps) {
  ...
  return <section className="tender-document-section" aria-labelledby="tender-document-title">
    <header><div><span className="eyebrow">Revisión documental</span><h3 id="tender-document-title">Documentos vigentes del proceso</h3><p>La actualización oficial versiona archivos sin generar un análisis nuevo. Los complementarios se cargan por separado.</p></div><strong>{documents.length} archivo(s)</strong></header>
    <div className="tender-document-actions" aria-label="Acciones documentales">
      {sourceUrl ? <a className="button secondary" href={sourceUrl} target="_blank" rel="noreferrer">Abrir fuente oficial</a> : <span className="button secondary is-disabled" aria-disabled="true">Fuente oficial no disponible</span>}
