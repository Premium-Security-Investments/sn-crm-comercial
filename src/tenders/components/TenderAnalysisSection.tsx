...
import { mergeTenderEvidence, normalizeTenderEvidence, tenderAnalysisMethodLabel, tenderAnalysisProducerDisclosure, tenderCurrentDocumentCount, tenderDecisionStatusTone, tenderNextAction, type TenderAnalysisOpportunityMetadata } from '../tenderDecisionBrief';
...
import './tender-analysis-hierarchy.css';
...
type TenderAnalysisSectionProps = {
  ...
  /** Real opportunity metadata only — never fabricated; fields with no persisted source render as unavailable. */
  metadata?: TenderAnalysisOpportunityMetadata;
};
...
export function TenderAnalysisSection({ analysis, documents, busy, canRunPreview, onAnalyzePreview, statusText = '', statusTone = 'status', analysisEngine, questionResponses = [], canAnswerQuestions = false, onSaveQuestionResponse, processingStatus = null, onRetryProcessing, metadata }: TenderAnalysisSectionProps) {
  const strengths = analysis?.strengths ?? analysis?.commercial_fit?.positives ?? [];
  const weaknesses = mergeTenderEvidence(analysis?.weaknesses, analysis?.blockers, analysis?.commercial_fit?.concerns);
  ...
  const currentDocumentCount = tenderCurrentDocumentCount(documents);
  return <section id="tender-analysis" className="tender-analysis-section tender-detail-anchor" aria-labelledby="tender-analysis-title" tabIndex={-1}>
    ...
    {analysis && analysis.status !== 'failed' && <>
      <header className="tender-analysis-hero" aria-label="Identificación real del expediente analizado">
        <div>
          <div className="tender-analysis-kicker"><span>{tenderAnalysisMethodLabel(analysis.producer)}</span><span>{analysis.current ? 'Vigente' : 'Obsoleto'}</span></div>
          <dl className="tender-analysis-run-meta">
            <div><dt>Entidad</dt><dd>{metadata?.entity || 'No disponible'}</dd></div>
            <div><dt>Proceso</dt><dd>{metadata?.processReference || 'No disponible'}</dd></div>
            <div><dt>Cierre oficial</dt><dd>{metadata?.expectedCloseDate ? new Intl.DateTimeFormat('es-CO', { dateStyle: 'medium' }).format(new Date(`${metadata.expectedCloseDate.slice(0, 10)}T12:00:00`)) : 'No disponible'}</dd></div>
            <div><dt>Cuantía</dt><dd>{metadata?.offerValue != null ? new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(metadata.offerValue) : 'No disponible'}</dd></div>
            <div><dt>Documentos actuales</dt><dd>{currentDocumentCount}</dd></div>
            <div><dt>Críticos abiertos</dt><dd>{analysis.critical_open_count ?? 'No disponible'}</dd></div>
          </dl>
        </div>
      </header>
      <article className="tender-decision-brief" aria-label="Conclusión preliminar de licitación">
