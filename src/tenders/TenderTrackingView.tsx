...
export function TenderTrackingView({ data, refresh, request, navigate }: TendersModuleProps) {
  ...
  const openEditor = (tender: PublicTender) => {
    setEditingId(tender.id); setForm(startForm(tender, data.currentProfile.id)); setError(null); setNotice(null);
  };
  const save = async () => {
  ...
  const convert = async (tender: PublicTender) => {
    ...
    setSavingId(tender.id); setError(null); setNotice(null);
    try {
      ...
      setNotice(message);
      sessionStorage.setItem(`tender-conversion-${result.id}`, JSON.stringify({ document_import_status: result.document_import_status, document_import_error: result.document_import_error, message }));
      await refresh();
