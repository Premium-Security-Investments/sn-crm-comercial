...
function OpportunityDetail({ id, data, refresh }: { id: string; data: Bootstrap; refresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setDetail(null); setError(null); setExitFeedback(null); setExitingTender(null); setTenderAnalysis(null); setTenderRevision(0);
  ...
  return <section className="stack">
    ...
    {o.service_type_code === 'licitacion_publica' ? <>
      <Panel title="Resumen de la oportunidad">
        <div className="tender-opportunity-summary-grid">
          ...
          <Info label="Cuantía" value={fmtMoney(o.offer_value)}/><Info label="Cierre oficial" value={fmtDateOnly(o.expected_close_date)}/><Info label="Días restantes" value={tenderDaysRemainingLabel(o.expected_close_date)}/><Info label="Etapa" value={o.stage_name}/><Info label="Responsable" value={o.owner_name || 'Sin comercial'}/>
        </div>
    ...
    {o.service_type_code === 'licitacion_publica' && <div id="tender-decision" className="tender-detail-anchor">
      <TenderDecisionBrief analysis={tenderAnalysis} commercialContext={{ amountLabel: fmtMoney(o.offer_value), closeLabel: fmtDateOnly(o.expected_close_date), daysLabel: tenderDaysRemainingLabel(o.expected_close_date), city: o.quote_city, sector: o.economic_sector, commercialFitPositives: tenderAnalysis?.commercial_fit?.positives || [] }} />
      <TenderGoNoGoDecisionPanel key={`tender-decision-${o.id}`} opportunityId={o.id} opportunityName={o.company_name || 'Oportunidad de licitación'} analysis={tenderAnalysis} currentProfile={data.currentProfile} request={api} onNavigationStateChanged={state => { if (activeDetailIdRef.current === o.id) setTenderDecisionNavigationState(state); }} onChanged={async () => { await load(); await refresh(); if (activeDetailIdRef.current === o.id) setTenderRevision(revision => revision + 1); }} />
    </div>}
...
function TenderDocumentReviewPanel({ opportunity, currentProfile, onReload, onAnalysisChanged, onNavigationStateChanged, focusTargetRef }: { opportunity: Opportunity; currentProfile: Profile; onReload?: () => Promise<void>; onAnalysisChanged?: (analysis: TenderDocumentAnalysis | null) => void; onNavigationStateChanged?: (documents: TenderPanelState<TenderDocumentNavigationValue>, analysis: TenderPanelState<TenderDocumentAnalysis | null>) => void; focusTargetRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  return <div id="tender-document-review" className="tender-guided-review" tabIndex={-1} ref={focusTargetRef}>
    ...
    <details className="tender-integral-analysis-trace" id="tender-integral-analysis-v3"><summary>Ver análisis técnico y trazabilidad por requisito</summary><TenderIntegralAnalysisV3View analysis={analysis} /></details>
  </div>;
}
