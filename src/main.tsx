...
function TenderCompanyProfilePanel() {
  ...
  return <section className="company-compliance-panel company-profile-panel" aria-label="Información de empresa para analizar licitaciones">
    <details className="company-profile-details">
      <summary><div><span className="eyebrow">Empresa licitante</span><strong>Información real de la empresa</strong><p>{profileUpdatedLabel} · {filled}/{tenderCompanyProfileFields.length} campos</p></div><span className="button secondary company-profile-summary-action">Abrir / editar ficha de empresa</span></summary>
      <div className="company-profile-body">
        <div className="company-compliance-head"><div><h2>Información real de la empresa</h2><p>Cargue el RUP y revise los campos clave.</p></div><label className="rup-upload-card"><span>Cargar RUP actualizado</span><input type="file" accept=".pdf,.docx,.txt,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain" disabled={uploadingRup || loadingProfile} onChange={event=>{ uploadRup(event.currentTarget.files?.[0]); event.currentTarget.value = ''; }}/><small>{uploadingRup ? 'Procesando documento…' : 'PDF, DOCX o TXT'}</small>{profileStatus ? <strong className={`rup-upload-status ${profileStatus.toLowerCase().includes('error') ? 'error' : ''}`}>{profileStatus}</strong> : null}</label></div>
