...
function TendersRadar({ data, refresh }: { data: Bootstrap; refresh: () => Promise<void> }) {
  ...
  return <section className="stack tenders-page">
    ...
    <section className="tender-control-panel" aria-label="Controles de licitaciones">
      ...
      <div className="filters tender-refine-filters"><span className="filter-label">Refinar resultados</span><Select value={primaryTenderFilter} onChange={v=>applyPrimaryTenderFilter(v as TenderPrimaryFilter)} options={[placeholderOption('Prioridad / filtro'),["hacer","Prioridad / filtro: Hacer hoy"],["revisar","Prioridad / filtro: Revisar"],["descartar","Prioridad / filtro: Descartar / validar"],["nuevas","Prioridad: Nuevas"],["urgentes","Prioridad: Urgentes"],["alto_valor","Prioridad: Alto valor"],["alto_encaje","Prioridad: Alto encaje"]]} empty="Prioridad / filtro"/><Select value={internalStatus} onChange={v=>{ setInternalStatus(v as TenderInternalStatus | 'todas'); setQuickFilter(null); setFastFilter(null); }} options={[placeholderOption('Estado interno'),["nueva","Estado interno: Nueva"],["en_revision","Estado interno: En revisión"],["descartada","Estado interno: Descartada"],["convertida_oportunidad","Estado interno: Convertida"]]} empty="Estado interno"/><Select value={sourceFilter} onChange={setSourceFilter} options={[placeholderOption('Fuente'), ...sourceOptions]} empty="Fuente"/><Select value={deadlineFilter} onChange={v=>setDeadlineFilter(v as TenderDeadlineFilter)} options={[placeholderOption('Cierre'),["0_7","Cierre: 0-7 días"],["8_15","Cierre: 8-15 días"],["16_30","Cierre: 16-30 días"],["vencida","Cierre: Vencida"],["sin_fecha","Cierre: Sin fecha"]]} empty="Cierre"/><Select value={valueFilter} onChange={v=>setValueFilter(v as TenderValueFilter)} options={[placeholderOption('Valor'),["sin_valor","Valor: Sin valor"],["lt_50m","Valor: <$50M"],["50m_500m","Valor: $50M-$500M"],["500m_plus","Valor: $500M+"],["1000m_plus","Valor: $1.000M+"]]} empty="Valor"/><Select value={scoreFilter} onChange={v=>setScoreFilter(v as TenderScoreFilter)} options={[placeholderOption('Encaje'),["alto","Encaje: Alto"],["medio","Encaje: Medio"],["bajo","Encaje: Bajo / validar"]]} empty="Encaje"/><button className="secondary" onClick={clearTenderFilters}>Limpiar filtros</button></div>
    </section>
