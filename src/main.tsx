...
import { TenderDecisionBrief } from './tenders/components/TenderDecisionBrief';
...
function parseRoute(): Route {
  ...
  if (page === 'edit') return id ? { page: 'edit', id: decodeURIComponent(id) } : { page: 'invalid' };
  if (page === 'consultant') return id ? { page: 'consultant', id: decodeURIComponent(id) } : { page: 'invalid' };
...
function fmtDateOnly(value?: string | null) { return formatDateOnly(value); }
// La vigencia temporal de la oportunidad se resuelve una sola vez, en TenderDetailNavigation
// (`resolveTenderValidity` → Vigente / Vencida). El Resumen conserva la fecha de cierre oficial y
// no vuelve a imprimir un conteo regresivo capaz de degradar a una segunda alerta de vencimiento.
...
function RouterView({ route, data, refresh }: { route: Route; data: Bootstrap; refresh: () => Promise<void> }) {
  if (route.page === 'invalid') return <div className="error">URL inválida o sección no encontrada.</div>;
...
function OpportunityDetail({ id, data, refresh }: { id: string; data: Bootstrap; refresh: () => Promise<void> }) {
  ...
  const [tenderQuestionResponses, setTenderQuestionResponses] = useState<TenderQuestionResponse[]>([]);
  ...
  useEffect(() => {
    ...
    setDetail(null); setError(null); setExitFeedback(null); setExitingTender(null); setTenderAnalysis(null); setTenderQuestionResponses([]); setTenderRevision(0);
  ...
  const exitTender = async (destination: 'radar' | 'seguimiento') => {
    ...
    if (!confirmed) return;
    const reason = window.prompt(destination === 'radar' ? 'Motivo para volver la licitación al Radar (opcional):' : 'Nota para continuar el seguimiento (opcional):', '');
    if (reason === null) return;
    setExitingTender(destination); setExitFeedback(null);
    try {
      await exitTenderOpportunity(o.id, destination, reason);
    } catch (err) {
      setExitFeedback(err instanceof Error ? err.message : String(err));
      setExitingTender(null);
      return;
    }
    // The transition is already committed. A transient bootstrap refresh failure
    // must not present it as failed or invite a duplicate exit attempt.
  ...
  return <section className="stack">
    ...
    {o.service_type_code === 'licitacion_publica' && <div id="tender-decision" className="tender-detail-anchor" tabIndex={-1}>
      ...
      <TenderGoNoGoDecisionPanel key={`tender-decision-${o.id}`} opportunityId={o.id} opportunityName={o.company_name || 'Oportunidad de licitación'} analysis={tenderAnalysis} currentProfile={data.currentProfile} request={api} questionResponses={tenderQuestionResponses} onNavigationStateChanged={state => { if (activeDetailIdRef.current === o.id) setTenderDecisionNavigationState(state); }} onChanged={async () => { await load(); await refresh(); if (activeDetailIdRef.current === o.id) setTenderRevision(revision => revision + 1); }} />
    </div>}
...
function TenderDocumentReviewPanel({ opportunity, currentProfile, onReload, onAnalysisChanged, onQuestionResponsesChanged, onNavigationStateChanged, focusTargetRef }: { opportunity: Opportunity; currentProfile: Profile; onReload?: () => Promise<void>; onAnalysisChanged?: (analysis: TenderDocumentAnalysis | null) => void; onQuestionResponsesChanged?: (responses: TenderQuestionResponse[]) => void; onNavigationStateChanged?: (documents: TenderPanelState<TenderDocumentNavigationValue>, analysis: TenderPanelState<TenderDocumentAnalysis | null>) => void; focusTargetRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  const addFiles = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPayload(data); onAnalysisChanged?.(data.analysis || null); onQuestionResponsesChanged?.(data.question_responses || []); emitNavigationPayload(data); setStatusText('Documentos guardados en Supabase y texto extraído.');
  ...
  const analyzeDocumentsWithAgt002 = async () => {
    ...
    try {
      ...
      setPayload(data); onAnalysisChanged?.(data.analysis || null); onQuestionResponsesChanged?.(data.question_responses || []);
  ...
  const saveQuestionResponse = async (input: TenderQuestionResponseInput, files: File[] = []) => {
    ...
    try {
      ...
      onQuestionResponsesChanged?.(data.question_responses);
  ...
  // Documentos y Análisis son secciones hermanas del expediente: Análisis deja de vivir dentro del
  // ancla documental para poder ser la superficie completa de condiciones e impedimentos.
  return <>
    <div id="tender-document-review" className="tender-guided-review" tabIndex={-1} ref={focusTargetRef}>
...
function UsersAdmin({ currentProfile }: { currentProfile: Profile }) {
  ...
  const load = async () => {
    ...
    try {
      const [nextUsers, nextCatalog] = await Promise.all([api<Profile[]>('/api/users'), api<AccessCatalog>('/api/access-catalog')]);
      setUsers(nextUsers);
  ...
  const moduleNamesByCode = new Map(MODULE_PERMISSIONS.map(module => [module.code, module.name]));
  const modulePermissions = catalog.permissions
    .filter(permission => MODULE_PERMISSION_CODES.includes(permission.code))
    .map(permission => ({ ...permission, name: moduleNamesByCode.get(permission.code) || permission.name }));
  const eligibleModuleCodes = new Set(eligibleModulePermissions(form.role));
  const eligibleModules = modulePermissions.filter(permission => eligibleModuleCodes.has(permission.code));
  ...
  return <section className="stack">
    ...
    <div ref={editFormRef} className="users-edit-anchor"><Panel title={editingUserId ? `Editar usuario · ${form.full_name || form.microsoft_email}` : 'Crear usuario'}><form className="form gridform" onSubmit={submit}>
      ...
      <fieldset className="access-permission wide">
        ...
        <p>Selecciona solo los módulos que esta persona debe ver y abrir. El rol define el techo, pero no asigna módulos automáticamente.</p>
        {catalogStatus === 'loading' && <p className="muted">Cargando catálogo de módulos…</p>}
        {catalogStatus === 'error' && <p className="error">No fue posible cargar el catálogo de módulos. Actualiza la página antes de guardar.</p>}
        {catalogStatus === 'ready' && eligibleModules.map(permission => <label className="checkline" key={permission.code}>
