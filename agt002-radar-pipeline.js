...
export function createAgt002RadarPipeline({
 ...
 return Object.freeze({async runOnce(){
  ...
  const stages=[];let rows,nowIso,evaluationDate;try{nowIso=now();if(typeof nowIso!=='string'||!Number.isFinite(Date.parse(nowIso)))throw new Error('invalid injected time');evaluationDate=agt002RadarEvaluationDate(nowIso);stages.push('fetch');rows=await fetchTenderPage(database,{limit:maxTendersPerRun});if(!Array.isArray(rows))throw new Error('fetch did not return rows');}catch{return{status:'unavailable',stages,error_code:'provider_error'};}
  ...
  try{stages.push('claim');for(const item of survivors){const attemptKey=computeAgt002RadarPreanalysisIdempotencyKey({kind:'attempt',tender_id:item.row.id,gate_evaluation_id:item.gateEvaluation.id,requested_at:nowIso});let outcome;try{outcome=await enqueueJob(database,{tenderId:item.row.id,gateEvaluationId:item.gateEvaluation.id,attemptKey,idempotencyKey:computeAgt002RadarPreanalysisIdempotencyKey({kind:'job',attempt_key:attemptKey}),policyVersion:item.evaluation.policy_version,contextVersion:item.evaluation.context_version,sourceRowHash:item.evaluation.source_row_hash});}catch{rejected+=1;continue;}if(outcome?.status==='satisfied')satisfied+=1;else if(outcome?.status==='created'||outcome?.status==='existing')enqueued+=1;else rejected+=1;}job=await claimJob(database,{leaseSeconds});}catch{return{status:'unavailable',stages,evaluated:evaluated.length,survivors:survivors.length,eliminated,enqueued,satisfied,rejected,error_code:'persistence_failure'};}
  const base={evaluated:evaluated.length,survivors:survivors.length,eliminated,enqueued,satisfied,rejected};if(!job)return{status:'empty',stages,...base};
  try{
