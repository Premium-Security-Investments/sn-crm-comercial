...
function validCandidate(value){return value&&typeof value==='object'&&key(value.tender_id)&&Array.isArray(value.service_terms)&&value.service_terms.every(item=>key(item))&&value.territory_key&&typeof value.territory_key==='object';}
...
export function buildAgt002RadarLearningSignals({candidate,observations,maxSignals}={}){
 ...
 for(const observation of observations.precedents){
  if(!observation||!key(observation.observation_id)||seen.has(observation.observation_id)||!Array.isArray(observation.evidence)||!observation.evidence.length||!['favorable','desfavorable','neutra'].includes(observation.signal_polarity))invalid('invalid or duplicate observation');seen.add(observation.observation_id);
