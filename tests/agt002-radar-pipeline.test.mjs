...
// Medición del puente tal como el runtime la fija en el envelope (issue #136). El pipeline no puede
// completarla desde el entorno, así que un `usage` vacío ya no es una entrada válida de este camino
// feliz; la autoridad y el fail-closed se prueban en agt002-radar-preanalysis-usage-authority.
...
for (const [label,evaluation] of [
 ...
 assert.deepEqual(staleResult.stages,['esu_refresh','fetch','gate','ledger','claim'],label);
}
