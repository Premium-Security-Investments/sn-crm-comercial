...
for (const source of [server, api]) {
  ...
  assert.match(source, /documents: includeExtractedText \? compatibleDocuments : compatibleDocuments\.map\(document => publicTenderDocumentProjection\(\{ \.\.\.document, analysis_suggestion: suggestAgt002DocumentRelevance\(document\) \}, \{ opportunityId \}\)\)/, 'la respuesta pública debe omitir el texto integral, anotar la sugerencia segura y proyectarse con la oportunidad ya autorizada');
