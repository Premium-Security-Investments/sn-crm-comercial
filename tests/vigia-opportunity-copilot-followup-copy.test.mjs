...
assert.ok(
  component.includes("const showForm = state.phase === 'idle' || state.phase === 'loading';"),
  'el formulario de preparación sólo se renderiza antes de tener un borrador (ready)',
);
assert.ok(component.includes('>Generar seguimiento<'), 'el CTA del formulario de preparación genera el seguimiento');
assert.ok(component.includes("onChangePreparation={() => setState(current => changeCopilotPreparation(current))}"), 'en ready la propuesta puede volver a la preparación');
assert.ok(component.includes('>Cambiar preparación<'), 'la propuesta ofrece su propio control para volver a la preparación');
assert.ok(
