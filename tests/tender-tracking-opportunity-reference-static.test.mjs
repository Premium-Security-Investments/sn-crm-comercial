...
for (const relative of ['../server/index.js', '../api/[...path].js']) {
  ...
  assert.match(source, /const tender = await getTenderTrackingTender\(database, tenderId\);[\s\S]{0,500}\.eq\('tender_id', tender\.id\)/, `${relative}: el historial debe consultar el tender canónico`);
