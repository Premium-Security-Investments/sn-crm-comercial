...
assert.match(boardReadonly, /if \(!payload\) \{\s*if \(!loading\)/, 'Any completed request without a payload must enter the error state regardless of error message text.');
assert.match(boardReadonly, /status \|\| 'Error de conexión\.'/,'Network and decode failures must always render an actionable fallback error.');
assert.match(boardReadonly, /Cargando reportes publicados/, 'The Junta view must provide a loading state only while loading is true.');
