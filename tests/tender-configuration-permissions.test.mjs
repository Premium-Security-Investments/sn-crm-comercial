...
const { canConfigureTenders } = await import(permissionsUrl);
const human = (role, permissions = ['licitaciones']) => ({ id: `user-${role}`, role, active: true, identity_type: 'human', permissions });
for (const scenario of [
  { name: 'admin con permiso configura', profile: human('admin'), expected: true },
  { name: 'gerencia con permiso configura', profile: human('gerencia'), expected: true },
  { name: 'director con permiso configura', profile: human('director'), expected: true },
