...
assert.match(panel, /id="tender-decision-register-go"(?![^>]*className="danger")/, 'Registrar GO conserva la jerarquía primaria.');
assert.match(panel, /id="tender-decision-register-nogo" className="danger"/, 'Registrar NO GO usa la variante destructiva secundaria.');
assert.match(styles, /button\.danger\{[^}]*border:/, 'La variante destructiva debe tener borde controlado.');
assert.match(styles, /button\.danger:hover/);
assert.match(styles, /button\.danger:focus-visible/);
