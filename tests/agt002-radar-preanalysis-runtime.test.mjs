...
// El cliente de puente real siempre devuelve una medición `usage` válida (la valida contra el
// transporte firmado), así que las respuestas simuladas la incluyen: lo que estos casos prueban es
// el JSON del modelo, no la medición. La autoridad de `usage` se prueba en
// tests/agt002-radar-preanalysis-usage-authority.test.mjs.
const BRIDGE_USAGE = { input_tokens:0, output_tokens:0 };
const invalid = createAgt002RadarPreanalysisRuntime({ environment:env, createClient:() => ({run:async () => ({output:{agent_id:'AGT-003'},usage:BRIDGE_USAGE})}) });
...
const wrongPolicy = createAgt002RadarPreanalysisRuntime({environment:env,createClient:()=>({run:async () => ({output:gatePolicyOutput,usage:BRIDGE_USAGE})})});
