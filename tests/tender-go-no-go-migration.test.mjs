...
assert.match(sql, /create unique index if not exists psi_sales_interactions_tender_offer_preparation_unique/i);
assert.match(sql, /where interaction_type = 'tender_offer_preparation'/i);
...
assert.match(sql, /decided_by uuid not null[^,\n]*references public\.psi_sales_profiles/i, 'la tabla nueva exige actor de auditoría');
...
assert.doesNotMatch(sql, /grant\s+(?=[^;]*\binsert\b)[^;]*on\s+table\s+public\.psi_tender_go_no_go_decisions\s+to\s+service_role/i, 'el audit log no permite INSERT directo a service_role');
assert.match(sql, /on conflict \(opportunity_id\) where interaction_type\s*=\s*'tender_offer_preparation' do nothing/i, 'la idempotencia apunta solo al índice parcial de preparación');
