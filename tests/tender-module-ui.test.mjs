...
assert.match(nav, /title: 'Licitaciones'[\s\S]{0,220}href: '#\/tenders\?view=radar', label: 'Radar', page: 'tenders'/, 'El catálogo central debe conservar un único enlace padre hacia Radar.');
assert.doesNotMatch(nav, /Radar de oportunidades/, 'La navegación lateral no debe conservar el copy anterior de Radar.');
...
for (const label of ['Radar', 'Seguimiento', 'Oportunidades']) {
  assert.match(tabsSource, new RegExp(`label: '${label}'`), `Las tabs del módulo deben incluir exactamente ${label}.`);
}
assert.doesNotMatch(tabsSource, /Radar de oportunidades/, 'Las tabs no deben conservar el copy anterior de Radar.');
