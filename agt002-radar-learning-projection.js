...
async function read(database,table,fields,limit){const response=await database.from(table).select(fields).limit(limit);if(response?.error)throw response.error;return response?.data||[];}
