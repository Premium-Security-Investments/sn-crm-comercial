...
| Migración | Rollback | Qué revierte |
|---|---|---|
| `077_agt002_canonical_persistence_statement_timeout.sql` | `077_agt002_canonical_persistence_statement_timeout_rollback.sql` | existe — devuelve `service_role` a la postura EXACTA de catálogo previa a 077 (`RESET` de `statement_timeout` y `lock_timeout`, sin entrada explícita para ninguno de los dos). No toca esquema, filas, grants, políticas RLS ni el cuerpo de `psi_record_agt002_canonical_analysis_run` |

**Línea base real, previa a 077:** `service_role` no tiene ninguna entrada explícita en `pg_db_role_setting`/`rolconfig`; su presupuesto de statement efectivo de 8s hoy lo aplica PostgREST por herencia, no un valor fijado en el catálogo. `anon` tiene 3s explícitos y `authenticated` tiene 8s explícitos — ninguno de los dos cambia con 077. El rollback de 077 hace `RESET` de ambos GUC gestionados, restaurando exactamente esa ausencia de catálogo — no fija `statement_timeout`/`lock_timeout` en `8s` ni en ningún otro valor. El comportamiento efectivo tras el rollback vuelve a lo que PostgREST/la sesión del autenticador apliquen por herencia, que hoy es 8s/8s; si ese valor heredado cambia en el futuro por fuera de este par migración/rollback, el rollback sigue ese nuevo valor en vez de sobreescribirlo de vuelta a 8s. Un GUC de rol no gestionado (distinto de `statement_timeout`/`lock_timeout`) que ya exista sobre `service_role` sobrevive intacto al rollback, porque el `RESET` sólo afecta a los dos GUC gestionados.
...
3. **`lock_timeout` nunca queda por encima de 8s en ninguna dirección.** La migración lo fija explícitamente en `8s`; el rollback lo `RESET`ea, y el valor heredado efectivo hoy también es `8s`. El presupuesto de statement ampliado sólo es seguro porque una espera real de lock sigue muriendo en 8s (fijado o heredado) con 55P03 en vez de colgarse 30s. `tests/agt002-canonical-persistence-statement-timeout-migration-static.test.mjs` falla si la migración fija `lock_timeout` por encima de 8s o si el rollback llega a fijarlo (en vez de `RESET`earlo) a cualquier valor.
4. **El rollback SÍ es una reversión exacta del catálogo para los dos GUC gestionados** — `RESET` deja a `service_role` sin entrada explícita de `statement_timeout`/`lock_timeout`, igual que antes de 077. No es una reversión exacta de "toda la fila": un GUC de rol no gestionado que ya existiera sobre `service_role` (p. ej. `work_mem`) sobrevive intacto, porque el rollback sólo toca los dos GUC gestionados.

Ambos archivos verifican su propio resultado dentro de la misma transacción contra `pg_db_role_setting` y abortan (fail-closed) si el resultado no es exactamente el esperado — el par `30s`/`8s` tras aplicar, o la ausencia de ambas entradas gestionadas tras revertir — o si existe un ajuste `in database` de `statement_timeout`/`lock_timeout` sobre `service_role` que anularía el ajuste a nivel de rol — un ajuste `in database` de otro GUC no relacionado no bloquea ni la migración ni el rollback.
