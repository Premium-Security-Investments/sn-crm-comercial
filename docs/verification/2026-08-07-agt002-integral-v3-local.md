...
Sequential gates re-run after this block (see `CURRENT.md` for the exact numbers): focal governance-overrides tests, `tests/agt002-*.test.mjs` regression, PGlite (`agt002-integral-governance-overrides-pglite.integration.test.mjs` + prior 063/canonicity suite), backend parity, build, `npm audit --omit=dev`, `git diff --check`, full suite baseline classification.
