...
- **[HOTFIX 2026-08-28 — ver `docs/superpowers/specs/2026-08-28-agt002-radar-derived-day-churn-hotfix.md`]** La afirmación de arriba —"este diseño no reimplementa el corto circuito `satisfied`: lo hereda sin cambios"— sigue siendo cierta para el RPC, pero **ya no describe sola la etapa `enqueue`**. El corto circuito del RPC compara `source_row_hash`, y el recolector diario externo reescribe `raw.days`/`raw.window`, que entran en ese hash: una fila sin cambio material cambia de identidad todos los días y el corto circuito deja de aplicar. Por eso la etapa `enqueue` incorpora, **antes** de encolar, un filtro transicional que descarta el churn reproducible cambiando **únicamente** esos dos campos derivados a una variante histórica válida. No añade etapa, no cambia el hash, ni la política, ni el contexto, ni el esquema. `createAgt002RadarScan` gana un solo parámetro inyectable (`readCanonicalPreanalysis`, carga **bulk** de canónicos, nunca N+1) y el resultado gana un solo contador (`satisfied_derived_only`), separado de `satisfied`.
- Retorno en el caso exitoso: `{status:'completed', stages, esu_refresh, evaluated, survivors, eliminated, enqueued, satisfied, rejected}` **[HOTFIX 2026-08-28: más `satisfied_derived_only`]**. No hay `status:'empty'` distinguible de `'completed'` con `evaluated:0`: a diferencia del worker, el scan siempre "hace su trabajo" (leer y evaluar la página) aunque no haya nada que encolar; `'empty'` se reserva para el worker, donde sí es una salida cualitativamente distinta (cero lecturas más allá de la reclamación).
