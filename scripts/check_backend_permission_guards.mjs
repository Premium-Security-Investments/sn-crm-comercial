...
for (const file of files) {
  ...
  assert.doesNotMatch(source, /function canViewTenders\(profile\)[^\n]*microsoft_email/, `${file}: tender guard must never authorize by email`);
  ...
  assert.match(source, /app\.get\('\/api\/users',[\s\S]*?requireAction\(currentProfile, ACTIONS\.USERS_MANAGE, \{\}\)/, `${file}: GET /api/users must require central USERS_MANAGE capability`);
}
