...
if(!url||!key){console.error(JSON.stringify({status:'unavailable',code:'AGT002_RADAR_ENTRYPOINT_CONFIG_INVALID'}));process.exitCode=1;}else{
 try{const database=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}});const pipeline=createAgt002RadarPipeline({database,environment:process.env,now:()=>new Date().toISOString()});const result=await pipeline.runOnce();console.log(JSON.stringify(result));}
