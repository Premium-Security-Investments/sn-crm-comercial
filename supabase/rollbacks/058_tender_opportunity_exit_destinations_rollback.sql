...
drop function if exists public.psi_exit_tender_opportunity(uuid, uuid, text, text, timestamptz);
...
-- We cannot blindly restore migration 018's original psi_update_tender_tracking and
-- psi_transition_tender_tracking bodies here. Those bodies assumed two invariants that only held
-- before migration 058 existed: (1) a tender in internal_status = 'nueva' always has a null
-- tracking_updated_at, and (2) converted_opportunity_id is always null outside
-- internal_status = 'convertida_oportunidad'. Migration 058's psi_exit_tender_opportunity
-- intentionally breaks both: "Sacar de oportunidad" leaves the Radar row with a non-null CAS token,
-- and both exit destinations preserve converted_opportunity_id so a later reconversion can reactivate
-- the same CRM opportunity instead of creating a duplicate. Rows already exited while 058 was live are
-- not rewritten by this rollback (no business data is deleted or reset), so restoring the literal 018
-- bodies would strand them: psi_update_tender_tracking would reject any non-null token on a 'nueva'
-- row with "La versión inicial de seguimiento debe ser nula.", and psi_transition_tender_tracking would
-- silently null out a preserved converted_opportunity_id instead of routing the caller to the exit RPC's
-- reconciliation. These rollback-compatible bodies keep the CAS/guard fixes from 058 so already-exited
-- rows stay operable after rollback, while everything else about the pre-058 tracking behavior is
-- unchanged.
...
begin
  ...
  -- A 'nueva' row can carry a non-null tracking_updated_at when migration 058's exit RPC returned it
  -- to Radar (see the note above psi_update_tender_tracking): accept an exact CAS match in that case
  -- instead of assuming the token is always null, while keeping the original null/null initial case.
  if v_tender.internal_status = 'nueva' then
    if v_tender.tracking_updated_at is null then
      if p_expected_tracking_updated_at is not null then
        raise exception 'La versión inicial de seguimiento debe ser nula.' using errcode = 'P0001';
...
revoke all on function public.psi_discard_tender_opportunity(uuid, uuid, text, timestamptz) from anon;
