...
-- Restore the pre-063 (025) append-only guard: no transition is allowed at all,
-- including canonical true -> false. Restore psi_record_agt002_canonical_analysis_run
-- to its pre-063 (056) body: plain insert-if-absent, no supersession. Signatures are
-- unchanged across 025/056/063, so create-or-replace is enough for both; no
-- psi_tender_analysis_runs row is ever touched by this rollback.
...
drop index if exists public.psi_tender_analysis_runs_one_canonical_current_idx;
