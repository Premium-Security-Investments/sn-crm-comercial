...
-- Task 7 (integral v3 persistence, DB layer). 063 gave psi_tender_analysis_runs its
-- canonical-promotion RPC: one canonical=true, status='completed' row per opportunity,
-- demote-in-place supersession, an idempotency short-circuit that never re-runs
-- supersession, a same-opportunity row lock, and a partial unique index as the
-- mechanical backstop. This migration hardens that same RPC *in place* (signature,
-- locking, idempotency, promotion/supersedes and grants all preserved byte-for-byte from
-- 063) with one added, fail-closed gate for the integral v3 contract:
--
--   * A payload that self-identifies as v3 — p_schema_version = '3.0.0', the exact value
--     the v3 runtime emits (agt002-preview-persistence.js AGT002_INTEGRAL_V3_SCHEMA_VERSION
--     / agt002-preview-contract.js AGT002_INTEGRAL_ENVELOPE_SCHEMA_VERSION) — must carry a
--     structurally complete integral_analysis: contract_version 'agt002-integral-analysis-v3',
--     a coverage object, and an analysis_units array. A truncated/invalid one is rejected
--     BEFORE any lock, demotion or insert, so a malformed v3 run can never demote the
--     opportunity's existing canonical run.
--   * coverage.legal_corpus_version_id must agree exactly with the attributable corpus id:
--     JSON null when p_legal_corpus_version_id is null, or the exact UUID as text when one
--     is supplied. 056's published-corpus gate below is preserved unchanged.
--   * Symmetrically, a v2 payload may never smuggle an integral_analysis object under a
--     non-v3 schema_version.
--
-- v2 stays accepted and readable exactly as before; nothing about the table, its columns,
-- indexes or triggers changes here — only the RPC body is replaced (create or replace,
-- same 12-parameter signature as 063), so the 067 rollback restores 063's exact body.
...
begin
  ...
  -- 067: integral v3 payload gate. Runs up front — before the idempotency short-circuit,
  -- the opportunity/canonical row locks and the demote-then-insert below — so a truncated
  -- or forged v3 payload is rejected without ever touching supersession state. A replay of
  -- an already-recorded valid v3 payload re-passes these same checks and short-circuits
  -- unchanged; a conflicting payload under an existing key is still caught by 063's
  -- idempotency comparison further down. '3.0.0' is the exact schema_version the v3 runtime
  -- emits; the gate keys off it and off the mere presence of integral_analysis.
  if jsonb_exists(p_result, 'integral_analysis') and p_schema_version is distinct from '3.0.0' then
    raise exception 'Un p_result con integral_analysis exige schema_version 3.0.0 (V3): un payload V2 no puede transportar un análisis integral V3.' using errcode = '22023';
  end if;
  if p_schema_version = '3.0.0' then
    if jsonb_typeof(p_result -> 'integral_analysis') is distinct from 'object'
       or (p_result -> 'integral_analysis' ->> 'contract_version') is distinct from 'agt002-integral-analysis-v3'
       or jsonb_typeof(p_result -> 'integral_analysis' -> 'coverage') is distinct from 'object'
       or jsonb_typeof(p_result -> 'integral_analysis' -> 'analysis_units') is distinct from 'array' then
      raise exception 'El análisis integral V3 requiere integral_analysis con contract_version agt002-integral-analysis-v3, coverage objeto y analysis_units arreglo.' using errcode = '22023';
    end if;
    -- coverage.legal_corpus_version_id must mirror the attributable corpus id exactly: JSON
    -- null when none is supplied, or the exact UUID as text when one is. The published-corpus
    -- gate (056) is still applied below, so this is an additional coherence check, not a
    -- replacement for it.
    if p_legal_corpus_version_id is null then
      if jsonb_typeof(p_result -> 'integral_analysis' -> 'coverage' -> 'legal_corpus_version_id') is distinct from 'null' then
        raise exception 'coverage.legal_corpus_version_id debe ser null cuando el análisis integral no referencia una versión de corpus.' using errcode = '22023';
      end if;
    elsif (p_result -> 'integral_analysis' -> 'coverage' ->> 'legal_corpus_version_id') is distinct from p_legal_corpus_version_id::text then
      raise exception 'coverage.legal_corpus_version_id debe coincidir exactamente con la versión de corpus suministrada.' using errcode = '22023';
    end if;
  end if;
