...
-- Enables an atomic durable-phase commit + lease release. Old callers that do
-- not send release_lease keep the exact migration-034 behavior.
...
declare
  ...
  v_release_lease boolean := coalesce((p_patch->>'release_lease')::boolean, false);
begin
  ...
  update public.psi_tender_processing_jobs set
    ...
    lease_id = case when v_release_lease then null else lease_id end,
    lease_expires_at = case when v_release_lease then null else lease_expires_at end,
    updated_at = now()
  where id = p_job_id;

  return jsonb_build_object('status', 'ok', 'lease_released', v_release_lease);
end;
$$;
