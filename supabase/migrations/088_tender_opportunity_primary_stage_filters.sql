...
as $$
begin
  if p_filter is null or p_filter not in ('all', 'por_decidir', 'en_curso', 'cerradas', 'pending_decision', 'go_authorized', 'in_preparation', 'submitted', 'closed') then
  ...
  left join lateral (
    select case
      when coalesce(d.decision, '') = 'no_go' or coalesce(o.tender_offer_status, 'pendiente_decision') in ('cerrada_no_go', 'adjudicada', 'no_adjudicada') then 'cerradas'
      when coalesce(d.decision, '') <> 'go' then 'por_decidir'
      else 'en_curso'
    end as stage
  ) st on true
