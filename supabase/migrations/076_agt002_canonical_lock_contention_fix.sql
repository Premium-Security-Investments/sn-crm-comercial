...
-- 076: minimal fix for AGT-002 canonical persistence lock contention.
-- Reproduced on PostgreSQL 16: 067 completes a 7.37 MB payload in ~0.43 s without
-- contention, but exhausts statement_timeout=8s while an unrelated transaction holds
-- KEY SHARE on psi_sales_opportunities. The opportunity FOR UPDATE is redundant for
-- canonical serialization and conflicts with KEY SHARE. Replace it with a transaction
-- advisory lock scoped to AGT-002 + opportunity; preserve the 067 signature, V3 gate,
-- idempotency, relationship checks, canonical-row lock, atomic demote/insert and grants.
...
begin
  ...
  -- and promotion locks — so a truncated or forged v3 payload is rejected without ever touching supersession state. A replay of
  ...
  -- 076: serialize only AGT-002 canonical promotions. The former opportunity FOR UPDATE
  -- conflicted with ordinary KEY SHARE holders (for example, concurrent inserts into related
  -- tables) and could exhaust the caller's statement_timeout before any canonical mutation.
  -- The transaction advisory lock preserves per-opportunity demote/insert serialization without
  -- blocking on unrelated opportunity-row readers. The FK graph protects opportunity identity.
  perform pg_advisory_xact_lock(hashtextextended('agt002-canonical:' || p_opportunity_id::text, 0));
  perform 1 from public.psi_sales_opportunities where id = p_opportunity_id;
